...
public class TargetTrackerRestControllerTest extends HealthometerTestSupport {
  ...
  void getValidTargetTracker() {
    assertEquals(
        1l,
        createGet(URL + "/1", TargetTrackerDto.class, TOKEN).getUserTargetCategoryDto().getId());
  }
  ...
  private TargetTrackerDto getTargetTrackerDto() {
    ...
    targetTrackerDto.setUnitValue(
        targetTrackerDto.getUnit().multiply(targetTrackerDto.getProductDto().getAmountValue()));
