...
public class UnitSubcategoryController {
    ...
    @PostMapping
    public ResponseEntity<UnitSubcategoryEntity> save(@RequestBody @NotNull UnitSubcategoryDto unitSubcategoryDto) {
        return new ResponseEntity<>(unitSubcategoryService.save(unitSubcategoryDto), HttpStatus.CREATED);
    }

    @PreAuthorize("hasAnyAuthority('admin')")
    @ApiOperation(value = "Update an existing Unit Subcategory")
    @PutMapping("/{id}")
    public ResponseEntity<UnitSubcategoryEntity> update(@RequestBody @NotNull UnitSubcategoryDto unitSubcategoryDto, @PathVariable @NonNull Long id) {
        return new ResponseEntity<>(unitSubcategoryService.update(id, unitSubcategoryDto), HttpStatus.CREATED);
    }
