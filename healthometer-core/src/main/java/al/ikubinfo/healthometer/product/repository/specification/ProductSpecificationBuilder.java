...
@Component
public class ProductSpecificationBuilder extends SpecificationBuilder<ProductEntity, ProductCriteria> {
    @Override
    public Specification<ProductEntity> filter(ProductCriteria criteria) {
        ...
        if (criteria.getName() != null) {
            specification = Objects.requireNonNull(specification).and(
                    likeLowerCaseSpecification("name", criteria.getName()));
        }
        if (criteria.getAmountValue() != null) {
            specification = Objects.requireNonNull(specification).and(
                    equalsSpecification("amountValue", criteria.getAmountValue()));
        }
        if (criteria.getTargetCategoryId() != null) {
            specification = Objects.requireNonNull(specification).and(
                    ((root, query, criteriaBuilder)
                            -> criteriaBuilder.equal(root.get("targetCategoryEntity").get("id"), criteria.getTargetCategoryId())));
        }
