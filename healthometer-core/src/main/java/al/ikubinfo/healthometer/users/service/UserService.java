...
public class UserService implements UserDetailsService {
  ...
  public MeasurementDto calculateBmi(Long userId) {
    ...
    BigDecimal heightValue =
            UnitConverter.convertFromLengthUnitToUnit(
                    height.getUnitSubcategoryEntity().getAbbreviation(),
                    "m",
                    height.getValue().doubleValue())
                    .round(new MathContext(4));
    BigDecimal weightValue =
            UnitConverter.convertFromMassUnitTo(
                    weight.getUnitSubcategoryEntity().getAbbreviation(),
                    "kg",
